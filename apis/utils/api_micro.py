...
def order_send_iiko(data,product_id):
    dict_tosend = {}
    dict_tosend['organizationId'] = str(data[0].branch_id)
    dict_tosend['terminalGroupId'] = str(data[0].order_br.terminal[0].id)
    dict_tosend['order'] ={}
    ...
    dict_tosend['order']['items'].append({"price":data[0].price,"type": "Product","amount": 1,'productId':product_id,"productSizeId": None,"comment": "string"})
    token = authiikocloud()
